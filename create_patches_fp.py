# The script "create_patches" is to extract patches and coordinates at the actual level.
# While this script is to save patches and coordinates at the patching (existed) level. 
# Coordinates should be always on level 0 to use openslide.read_region(), to check
# I modified the target_patch_size, so it could be also used for the case without custom downsampling, so one command for each slide format
# slide format was also then improve for strictly one command, but not tested
...
#                  patch = False, auto_skip=True, process_list = None, annotations=False, custom_downsample=1):
