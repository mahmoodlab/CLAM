...
def save_splits(split_datasets, column_keys, filename, boolean_style=False):
    ...
    if not boolean_style:
        ...
        df = pd.DataFrame(
            bool_array, index=index, columns=[
                'train', 'val', 'test'])
...
class Generic_WSI_Classification_Dataset(Dataset):
    ...
    def summarize(self):
        ...
        print(
            "slide-level counts: ",
            '\n',
            self.slide_data['label'].value_counts(
                sort=False))
        for i in range(self.num_classes):
            print(
    ...
    def create_splits(self, k=3, val_num=(25, 25), test_num=(
            40, 40), label_frac=1.0, custom_test_ids=None):
        settings = {
            'n_splits': k,
            'val_num': val_num,
            'test_num': test_num,
    ...
    def return_splits(self, from_id=True, csv_path=None):
        ...
        if from_id:
            ...
            assert csv_path
            # Without "dtype=self.slide_data['slide_id'].dtype", read_csv()
            # will convert all-number columns to a numerical type. Even if we
            # convert numerical columns back to objects later, we may lose
            # zero-padding in the process; the columns must be correctly read
            # in from the get-go. When we compare the individual train/val/test
            # columns to self.slide_data['slide_id'] in the get_split_from_df()
            # method, we cannot compare objects (strings) to numbers or even to
            # incorrectly zero-padded objects/strings. An example of this
            # breaking is shown in
            # https://github.com/andrew-weisman/clam_analysis/tree/main/datatype_comparison_bug-2021-12-01.
    ...
    def test_split_gen(self, return_descriptor=False):
        ...
        if return_descriptor:
            index = [
                list(
                    ...
                    list(
                        self.label_dict.values()).index(i)] for i in range(
                    self.num_classes)]
        ...
        for u in range(len(unique)):
            print(
                'number of samples in cls {}: {}'.format(
                    unique[u], counts[u]))
            if return_descriptor:
