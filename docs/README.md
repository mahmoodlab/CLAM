...
## Fork Info: This fork adds tifffile support to avoid issues with openslide/PIL related to tiff file formats. This allows for the user to process very large tiff files (>>4Gb). This repo will not be supported once full support for tifffile is integrated. Currently, tifffile support is limited to patch, mask, and stitch generation. ##
