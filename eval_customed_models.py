...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 20 17:37:18 2021
...
def get_dataset_loader(split_dataset, training = False, testing = False, weighted = False, batch_size = 64, trnsfrms = None):
    ...
    for i in range(len(split_dataset.slide_data['slide_id'])):
        with h5py.File(os.path.join(split_dataset.data_dir,split_dataset.slide_data['slide_id'][i])+'.h5','r') as hdf5_file:
            tile_IDs.extend([split_dataset.slide_data['slide_id'][i] +":" + str(s) for s in range(hdf5_file['imgs'].shape[0])]) # all tiles from each slides
            labels.extend([split_dataset.slide_data['label'][i]] * hdf5_file['imgs'].shape[0]) # split_dataset.slide_data['label']: 1,1,1,...,1,0,0,0,...,0
...
def summary(model, loader, n_classes): # should have tile level and slide level
    ...
    print('In total, there are {} patches.'.format(len(loader)))
...
if __name__ == "__main__":
    ...
    if args.task == 'tcga_hcc_test_cv_c2_622':
        args.n_classes=2
